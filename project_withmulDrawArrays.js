...
window.onload = function init() {
    ...
    lineButton.addEventListener('click', function () {
        objects.shapes.push({
            ...
            isSelected: function (vertices, clkPts) {
                ...
                if (lhs == rhs || lhs - rhs > 0 && lhs - rhs < 0.02 || lhs - rhs < 0 && lhs - rhs > -0.02) {
                    var Dx = vertices[1][0] - vertices[0][0];
                    var Dy = vertices[1][1] - vertices[0][1];
                    var D = Math.sqrt(Dx * Dx + Dy * Dy);
                    Dx = 0.5 * 0.04 * Dx / D;
                    Dy = 0.5 * 0.04 * Dy / D;
                    var boundingRect = [
                        vec2(vertices[0][0] - Dy, vertices[0][1] + Dx),
                        vec2(vertices[1][0] - Dy, vertices[1][1] + Dx),
                        vec2(vertices[1][0] + Dy, vertices[1][1] - Dx),
                        vec2(vertices[0][0] + Dy, vertices[0][1] - Dx),
                    ];
                    gl.useProgram(boundingProgram);
                    gl.bufferData(gl.ARRAY_BUFFER, flatten(boundingRect), gl.STATIC_DRAW);
                    gl.enableVertexAttribArray(vPosition);
                    gl.drawArrays(gl.LINE_LOOP, 0, boundingRect.length);
                }
            }
        });
        render();
    ...
    canvas.addEventListener("click", function (event) {
        ...
        for (var j = 0; j < objects.shapes.length; j++) {
            objects.shapes[j].isSelected(objects.shapes[j].vertices, t);
        }
    });
